...
class Node(object):
    ...
    def calc_output(self):
        ...
        #self.upstream相当于从上层过来的一个权值和x的列表
...
class Network(object):
    ...
    def train(self,labels,data_set,rate,iteration):
        ...
        for i in range(iteration):
            for d in range(len(data_set)):  #改成1,
    ...
    def predict(self,sample):
        ...
        self.result=[]#设置列表，存取预测结果
        for i in range(1,len(self.layers)):
            self.layers[i].calc_output()

        #至此，已经完成6个节点的输出运算，最后return 是为了检查梯度时调用
        ...
        for node in self.layers[-1].nodes[:-1]:
            self.result.append(node.output)
...
def _test():
    ...
    _network.train([[1]],[[1,1,1]],0.5,1)  #注意data_set和label的表示[[]]
    _network._print_pre()
    #_network.dump()
